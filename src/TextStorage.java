...
public class TextStorage {
    ...
    private AudioItem parseCatalogLine(String line) {
        ...
        return switch (type) {
            ...
            case "PODCAST" -> {
                ...
                if (ep == Integer.MIN_VALUE) yield null;
            ...
            case "ALBUM" -> {
                ...
                if (tracks == Integer.MIN_VALUE) yield null;
    ...
    public void saveSinglePlaylist(String path, Playlist p) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(path))) {
            bw.write("PLAYLIST|" + p.getName());
            bw.newLine();
            for (int id : p.getItemIds()) {
    ...
    public Playlist loadSinglePlaylist(String path) {
        File f = new File(path);
        if (!f.exists()){
            return null;
        }
